...
function StepAboutMe() {
	...
	const { personalInfo, selectedProjects, aboutMe, tone } = useSelector(
		(state) => state.builder,
	);
	...
	function handleGenerate() {
		const topLanguages = [
			...new Set(selectedProjects.map((p) => p.language).filter(Boolean)),
		];
		...
		generateMutation.mutate(
			...
			{
				onSuccess: (data) =>
					setValue("aboutMe", data.text, { shouldValidate: true }),
			},
		);
	}
	...
	return (
		<form className={styles.step} onSubmit={handleSubmit(onSubmit)}>
			...
			<p className={styles.subtitle}>
				Pick a tone and let AI draft a starting point you can edit.
			</p>
